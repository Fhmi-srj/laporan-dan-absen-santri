...
try {
    ...
    // Find siswa by nomor_induk or RFID
    $siswa = null;
    if ($nomorInduk) {
        $stmt = $pdo->prepare("SELECT * FROM siswa WHERE nomor_induk = ?");
    ...
    // Enrich with SPMB data (get nama_lengkap and no_wa_wali from SPMB)
    $siswaList = [$siswa];
    ...
    echo json_encode([
        ...
        'siswa_rfid' => $siswa['no_kartu_rfid'] ?? '',
        'status' => $status
    ]);
