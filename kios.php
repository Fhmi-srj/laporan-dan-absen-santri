...
// Get today's attendance for live display - JOIN with data_induk
$today = date('Y-m-d');
$recentStmt = $pdo->prepare("
    SELECT a.*, di.nisn as nomor_induk, di.kelas, di.nama_lengkap, di.jenis_kelamin,
           j.name as jadwal_name, j.type as jadwal_type
    FROM attendances a
...
$totalSiswa = $pdo->query("SELECT COUNT(*) FROM data_induk WHERE deleted_at IS NULL")->fetchColumn() ?: 0;
?>
<!DOCTYPE html>
<html lang="id">
                    ...
                    <?php foreach ($recentAttendances as $a): ?>
                        <div class="live-item" data-name="<?= strtolower(e($a['nama_lengkap'])) ?>"
                            ...
                            <div class="live-info">
                                ...
                                <div class="live-meta">
                                    Kelas <?= e($a['kelas']) ?> | <?= e($a['nomor_induk'] ?? '-') ?>
                                </div>
                            </div>
                            <div class="live-time">
