...
// Helper function to get pendaftaran data for siswa
function getSiswaWithPendaftaran($pdo, $spmb, $siswaIds)
{
    if (empty($siswaIds))
        return [];

    // Get siswa data
    $placeholders = str_repeat('?,', count($siswaIds) - 1) . '?';
    $stmt = $pdo->prepare("SELECT * FROM siswa WHERE id IN ($placeholders)");
    ...
    $placeholders2 = str_repeat('?,', count($pendaftaranIds) - 1) . '?';
    $spmbStmt = $spmb->prepare("SELECT * FROM pendaftaran WHERE id IN ($placeholders2)");
    $spmbStmt->execute($pendaftaranIds);
    $pendaftaranData = [];
    while ($row = $spmbStmt->fetch()) {
        $pendaftaranData[$row['id']] = $row;
    }
...
$stmtRecent = $pdo->prepare("
    SELECT ca.*, s.kelas, s.pendaftaran_id
...
// Get nama from SPMB for aktivitas
$pendaftaranIds = array_column($recentAktivitas, 'pendaftaran_id');
$pendaftaranData = [];
if (!empty($pendaftaranIds)) {
    $placeholders = str_repeat('?,', count($pendaftaranIds) - 1) . '?';
    $spmbStmt = $spmb->prepare("SELECT id, nama FROM pendaftaran WHERE id IN ($placeholders)");
    $spmbStmt->execute($pendaftaranIds);
    while ($row = $spmbStmt->fetch()) {
        $pendaftaranData[$row['id']] = $row['nama'];
    }
}
foreach ($recentAktivitas as &$akt) {
    $akt['nama_lengkap'] = $pendaftaranData[$akt['pendaftaran_id']] ?? '-';
}
unset($akt);
...
// Get nama from SPMB for late siswa
$latePendaftaranIds = array_column($lateSiswa, 'pendaftaran_id');
if (!empty($latePendaftaranIds)) {
    $placeholders = str_repeat('?,', count($latePendaftaranIds) - 1) . '?';
    $spmbStmt = $spmb->prepare("SELECT id, nama FROM pendaftaran WHERE id IN ($placeholders)");
    $spmbStmt->execute($latePendaftaranIds);
    $latePendaftaranData = [];
    while ($row = $spmbStmt->fetch()) {
        $latePendaftaranData[$row['id']] = $row['nama'];
    }
    foreach ($lateSiswa as &$late) {
        $late['nama_lengkap'] = $latePendaftaranData[$late['pendaftaran_id']] ?? '-';
    }
    unset($late);
}
