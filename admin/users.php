...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'delete' && !empty($_POST['id'])) {
        ...
        redirectWith('users.php', 'success', 'User berhasil dihapus!');
    }
    if ($_POST['action'] === 'reset_device' && !empty($_POST['id'])) {
        $pdo->prepare("DELETE FROM user_devices WHERE user_id = ?")->execute([$_POST['id']]);
        redirectWith('users.php', 'success', 'Device berhasil direset!');
    }
    if ($_POST['action'] === 'store') {
        ...
        redirectWith('users.php', 'success', 'User berhasil ditambahkan!');
    }
    if ($_POST['action'] === 'update' && !empty($_POST['id'])) {
        ...
        redirectWith('users.php', 'success', 'User berhasil diperbarui!');
    }
}

// Get users
$filterRole = $_GET['role'] ?? '';
$sql = "SELECT u.*, (SELECT COUNT(*) FROM user_devices ud WHERE ud.user_id = u.id) as device_count FROM users u";
if ($filterRole) {
    $sql .= " WHERE u.role = ? ORDER BY u.name ASC";
...
<div class="main-content">
    ...
    <div class="card-custom">
        <div class="p-3 border-bottom">
            <select class="form-select w-auto" onchange="location.href='users.php?role='+this.value">
