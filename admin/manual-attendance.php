...
// Get siswa list (enriched with SPMB)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'store') {
        ...
        if ($check->fetch()) {
            ...
            redirectWith('manual-attendance.php', 'success', 'Data absensi berhasil diperbarui!');
        } else {
            // Insert new
            ...
            redirectWith('manual-attendance.php', 'success', 'Data absensi berhasil ditambahkan!');
        }
    }
    if ($_POST['action'] === 'delete') {
        $pdo->prepare("DELETE FROM attendances WHERE id = ?")->execute([$_POST['id']]);
        redirectWith('manual-attendance.php', 'success', 'Data absensi berhasil dihapus!');
    }
}

// Get recent manual entries
$recentAttendances = $pdo->query("
    SELECT a.*, s.kelas, s.pendaftaran_id, j.name as jadwal_name
    FROM attendances a
